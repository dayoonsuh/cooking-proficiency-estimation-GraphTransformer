...
from models import STGCN_Skeleton
...
def main():
    ...
    for idx, (train_subjects, test_subjects) in enumerate(full_partition):
        ...
        in_channels = 3
        num_class = 2
        graph_args = {'layout':'mediapipe', 'strategy':'uniform','max_hop':1,'dilation':1}
        edge_importance_weighting = True if args.edge_importance_weighting == 1 else False

        
        model = STGCN_Skeleton(in_channels, args.inter_dim, args.temporal_kernel_size, num_class, graph_args, edge_importance_weighting)
        ...
        # model training
        trained_model, history, preds, labels = train_model_stgcn2(model.to(args.device), criterion, optimizer, args.apply_scheduler, scheduler, dataloaders, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edge_importance_weighting', type=int, default=1)
