...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, ft_size, time_len, domain):
        ...
        #print(self.time_len, self.joint_num, ft_size)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h_num, h_dim, input_dim, dp_rate,domain, window_size):
    ...
    def attention(self, query, key, value):
        ...
        # [batch, time, ft_dim)
        d_k = query.size(-1)
        #print(d_k, query.shape, key.shape)
        scores = torch.matmul(query, key.transpose(-2, -1)) \
                 / math.sqrt(d_k)

        #print(d_k, query.shape, key.shape, scores.shape)
        #exit()
        ...
        if self.domain is not None:
            ...
            if self.domain == "temporal":
                ...
                #print(scores.shape, self.s_mask.shape)
...
class ST_ATT_Layer(nn.Module):
    ...
    def __init__(self, input_dim, inter_dim, h_num, h_dim, dp_rate, domain, window_size):
        #input_size : the dim of input
        #output_size: the dim of output
        #h_num: att head num
        #h_dim: dim of each att head
        #time_len: input frame number
        #domain: spatial or temporal attention

        super(ST_ATT_Layer, self).__init__()

        self.pe = PositionalEncoding(input_dim, window_size, domain)
        
        self.attn = MultiHeadedAttention(h_num, h_dim, input_dim, dp_rate, domain, window_size) 

        self.ft_map = nn.Sequential(
                        nn.Linear(h_num * h_dim, inter_dim),
                        nn.ReLU(),
                        LayerNorm(inter_dim),
                        nn.Dropout(dp_rate),
                        )
